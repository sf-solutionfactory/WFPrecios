<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="tab3.aspx.cs" Inherits="WFPrecios.Precios.tabs.tab3" EnableEventValidation="true" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function autocompleta_m(id) {
            $("#" + id).autocomplete({
                source: function (request, response) {
                    ...
                    var spa = $('#txtSPART').val();
                    if (spa != null) {
                        var param = { mk: para.toUpperCase(), sp: spa };
                        $.ajax({
        ...
        function llenar_material(id, value) {
            var idd = id.split('-')[1];
            value = value.split(' ')[0];
            var col = "WGBEZ";
            var param = { ma: value, co: col };
            $.ajax({
                url: "../../catalogos.asmx/llena_matkl",
                data: JSON.stringify(param),
                dataType: "json",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                dataFilter: function (data) { return data; },
                success: function (data) {
                    document.getElementById("NAME1-" + idd).value = data.d;
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {

                }
            });
            cambiaMeins("MEINS-" + idd, "M2");
        }
...
<body>
    <form id="form1" runat="server">
        <div class="container">
                ...
                <div class="col s12 m12 l12">
                    <table style="position: relative; left: 50%; margin-left: -200px">
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <select class="cell03" name="txtVTWEG" runat="server" id="txtVTWEG" onchange="eligeVTWEG(this.value)">
                                </select></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <input class="cell07" type="text" name="txtKUNNR" id="txtKUNNR" runat="server" required="required" onchange="cambiaKunnr()" />
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <input class="cell07" type="text" id="txtMATKL" runat="server" />
                                <script type="text/javascript">autocompleta_m("txtMATKL");</script>
                            </td>
                        </tr>
                        <tr>
                            <td class="cell03">% Incremento</td>
                            <td>
                                <input class="cell06" type="text" id="P_inc" runat="server" onchange="cambiaPorc(this.id)" />
                            </td>
                            <td class="cell03">$ Incremento</td>
                            <td>
                                <input class="cell06" type="text" id="C_inc" runat="server" onchange="cambiaPorc(this.id)" />
                            </td>
                        </tr>
                        <tr>
                            <td class="cell03">% Decremento</td>
                            <td>
                                <input class="cell06" type="text" id="P_dec" runat="server" onchange="cambiaPorc(this.id)" />
                            </td>
                            <td class="cell03">$ Decremento</td>
                            <td>
                                <input class="cell06" type="text" id="C_dec" runat="server" onchange="cambiaPorc(this.id)" />
                            </td>
                        </tr>
                    </table>
            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <input type="button" value="Cargar" class="btn right" runat="server" onclick="cargar()" id="btnCargar" disabled="disabled" />
                    <div class="right" style="width: 10px;">&nbsp;</div>
                    <input type="button" value="Agregar Nuevo" class="btn right" runat="server" onclick="add()" id="btnAgregar" disabled="disabled" />
                </div>
            </div>
            <script type="text/javascript">
                function eligeVKORG(value) {
                    ...
                    var param = { pr: pr, vk: vk };
                    $.ajax({
                        url: "../../catalogos.asmx/llenaVtweg",
                        data: JSON.stringify(param),
                        dataType: "json",
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        ...
                        success: function (data) {
                            ...
                            var pl = document.getElementById("txtVTWEG");
                            ...
                            eligeVTWEG(document.getElementById("txtVTWEG").value);
                        },
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                            //alert(textStatus);
                        }
                ...
                function eligeVTWEG(value) {
                    ...
                    var vk = document.getElementById("txtVKORG").value;
                    var vt = document.getElementById("txtVTWEG").value;
                    var pr = document.getElementById("hidNumEmp").value;
                    var param = { pr: pr, vk: vk, vt: vt };
                    $.ajax({
                        url: "../../catalogos.asmx/llenaSpart",
                        data: JSON.stringify(param),
                        dataType: "json",
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        ...
                        success: function (data) {
                            ...
                            var pl = document.getElementById("txtSPART");
                            for (var i = 0; i < pl.length; i++) {
                                pl.remove(0);
                                i--;
                            }
                ...
                function cargar() {
                    ...
                    $('form').get(0).setAttribute('action', './tab3.aspx'); //this works
                    document.forms["form1"].submit();
                }
                function cambiaPorc(id) {
                    if (id == "P_inc") {
                        document.getElementById("P_dec").value = "";
                        document.getElementById("C_inc").value = "";
                        document.getElementById("C_dec").value = "";
                    } else if (id == "P_dec") {
                ...
                function cambiaPed(campo) {
                    ...
                    document.getElementById("NAME1-" + id).value = "";
                    var bban = false;
                    var tabla2 = valores.split('|');

                    var tabla = valores.split('|');
                    var nueva = "";
                    var pos = 0;
                    for (var i = 0; i < tabla.length - 1; i += 10) {
                        pos++;
                        if (pos == id) {
                            var pos1 = 0;
                            for (var j = 0; j < tabla2.length - 1; j += 10) {
                                pos1++;
                                if (pos != pos1) {
                                    if (valor.split(' ')[0].trim() == tabla2[j]) {
                                        if (tabla[j + 8] == "X") {
                                            bban = true;
                                        }
                                    } else {
                ...
                function cambiaPed2(id, valor) {
                    //var valor = campo.value;
                    id = id.split('-')[1];
                    var valores = document.getElementById("txtTabla").value;

                    document.getElementById("NAME1-" + id).value = "";
                    //document.getElementById("MEINS-" + id).value = "";
                    var bban = false;
                    var tabla2 = valores.split('|');
                    ...
                    for (var i = 0; i < tabla.length - 1; i += 10) {
                        ...
                        if (pos == id) {
                            ...
                            if (bban) {
                                nueva += "|";
                                document.getElementById("MATKL-" + id).value = "";
                            } else {
                                nueva += valor.split(' ')[0] + "|";
                            }
                        } else {
                ...
                function add() {
                    ...
                    if (posi == 1) {
                        ...
                        linea += "<tr><td class='tablahead'>Gpo de artículos</td><td class='tablahead'>Denominación</td><td class='tablahead'>Unidad</td>" +
                            ...
                            "<td class='tablahead'>Válido de</td><td class='tablahead'>Válido a</td><td class='tablahead'>E</td><td class='tablahead'></td></tr>";
                        $("#lblTabla").append(linea);
                    }
                    linea = "";
                    linea += "<tr id='tr-" + posi + "'>";

                    linea += "<td class='tablaCent'><input class='cell031' type='text' id='MATKL-" + posi + "' onchange='cambiaPed(this)' onblur='borraMatnr(this.id, this.value)' ondblclick='escalas(this.id)' /></td>";
                    linea += "<td id='trNAME-" + posi + "' ondblclick='escalas(this.id)'><input class='cell03' type='text' id='NAME1-" + posi + "' disabled='disabled'  /></td>";
                    ...
                    linea += "<%=monedas %>";
                    linea += "</select></td>";
                    linea += "<td class='tablaCent'><input class='cell031 datepicker' type='text' onchange='cambiaFechA(this.value, this.id);'  ondblclick='copiaA(this.value)' id='DATAB-" + posi + "' /></td>";
                    linea += "<td class='tablaCent'><input class='cell031 datepicker' type='text' onchange='cambiaFechB(this.value, this.id)'  ondblclick='copiaB(this.value)' id='DATBI-" + posi + "' /></td>";
                    ...
                    //document.getElementById("txtTabla").value += "|0.00||0.00|MXN||||X|M|";//ADD RSG 13.09.2018
                ...
                function elimina(id) {
                    ...
                    var arr = tabla.split('|');
                    var temp = "";
                    for (var i = 0; i < arr.length - 1; i += 10) {
                        if ((i / 10 + 1) == num) {
                            temp += arr[i] + '|';
                            temp += arr[i + 1] + '|';
                            temp += arr[i + 2] + '|';
                ...
                function escalas(id) {
                    ...
                    var matnr = document.getElementById("MATKL-" + num).value;
            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    ...
                    <input type="hidden" runat="server" id="txtTipo" value="3" />
            ...
            function enviar() {
                ...
                for (var i = 0; i < tabla.length - 1; i += 10) {
                    if (tabla[i + 8] == "X") {
                        ...
                        if (tabla[i].trim() == "")
                            error = true;
                        if (document.getElementById("NAME1-" + ((i / 10) + 1)) != undefined)
                            if (document.getElementById("NAME1-" + ((i / 10) + 1)).value == "")
                                error3 = true;
                        if (tabla[i + 3].trim() == "")
                            error = true;
                        if (tabla[i + 5].trim() == "")
                            ...
                            revisaFechaA("DATAB-" + ((i / 10) + 1));
                            if (!error2)
                                error2 = revisaFechaA("DATAB-" + ((i / 10) + 1));
                        }
                        if (tabla[i + 6].trim() == "")
                            error = true;
                        else {
                            ...
                            revisaFechaA("DATAB-" + ((i / 10) + 1));
                            if (!error2)
                                error2 = revisaFechaA("DATBI-" + ((i / 10) + 1));
                        }
                        for (var j = 0; j < escala.length; j++) {
                            if (escala[j].id == (i / 10) + 1) {
