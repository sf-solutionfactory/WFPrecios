        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (con.conectar())
                {
                    ...
                    IRfcTable tab = con.tablaCabecera("01", "", hidNumEmp.Value);

                    for (int i = 0; i < tab.Count; i++)
                    {
                        tab.CurrentIndex = i;
                        string estatus = con.estatusSol("01", tab.GetString("ID_SOLICITUD"));
                        //switch (estatus)
                        //{
                        //    case "P":
                        //        estatus = "Pendiente";
                        ...
                        autorizante = con.porAutorizar("01", tab.GetString("ID_SOLICITUD"));
                        ...
                        if (!autorizante.Equals(""))
                        {
                            ...
                            {
                                table += "<td class='tablaCent'><a class='tablaLink' href='Procesa.aspx?Tipo=01&Folio=";
                                table += tab.GetString("ID_SOLICITUD");
                                table += "&Oper=N&Posi=";
                                table += con.sigFolio("01", tab.GetString("ID_SOLICITUD"));
                            }
                            else
                            {
